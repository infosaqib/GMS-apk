  ...
  try {
    ...
    productMenu.addEventListener("change", () => {
      ...
      cleaningCheckbox.addEventListener('change', function () {
        if (selectedProduct) {
          let cleaningPrice = Number(selectedProduct.cleaning_price); // Get base cleaning price
          let weight = Number(remainingWeight.value); // Get remaining weight
          let cleaningCost = cleaningPrice * weight; // Calculate cost
      
          if (this.checked) {
            // Set cleaning price and add it to total price
            cleaningPriceInput.value = cleaningPrice.toFixed(2);
            totalPrice.value = parseFloat(Number(totalPrice.value) + cleaningCost).toFixed(2);
          } else {
            // Subtract the cleaning cost when unchecked
            totalPrice.value = parseFloat(Number(totalPrice.value) - cleaningCost).toFixed(2);
