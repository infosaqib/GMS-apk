var Utils = (() => {
  ...
  __export(utils_exports, {
    ...
    closeClientInvoice: () => closeClientInvoice,
    closeVendorInvoice: () => closeVendorInvoice,
    cnicFormat: () => cnicFormat,
    contactFormat: () => contactFormat,
    openClientInvoice: () => openClientInvoice,
    openVendorInvoice: () => openVendorInvoice
  });
  ...
  function animateCounters(elements, options = {}) {
    const {
      threshold = 0.5,
      rootMargin = "0px",
      duration = 2e3
...
@media (min-width: 768px) {
  ...
  var invoiceView = document.getElementById("invoice-view");
...
`;
  async function openClientInvoice(event) {
  ...
  async function openVendorInvoice(event) {
    if (!event || !event.target) {
      console.error("Invalid event object");
      return;
    }
